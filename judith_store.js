...
while (true) {
  ...
  let custBag = prompt(
    "Please choose the type of bag you want or press Q to quit: "
  );
  if (custBag === "Q") {
    console.log(
      `Oops!! Thanks dear ${cust}, you're welcome to shop with us next time.!`
    );
    break;
  }
  let exist = store.find(
    (item) =>
      item.name.toLowerCase() === custBag.toLowerCase() ||
      item.name.slice(0, -4).toLowerCase() === custBag.toLowerCase()
  );
  if (!exist) {
    ...
    while (true) {
      let col = prompt(
        `Please choose color ${exist.color} or press Q to quit: `
      );
      if (col === "Q") {
        console.log(
          `Oops!! Thanks dear ${cust}, you're welcome to shop with us next time.!`
